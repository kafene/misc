 ...
 *   Something like:
       $bookmark_file = tidy_parse_string($bookmark_file)
       $bookmark_file->cleanRepair();
 */
function parse_netscape_bookmarks($bookmark_file, $default_tag = null) {
  $i = 0;
  $next = false;
  $items = array();
  $current_tag = $default_tag = $default_tag ?: 'imported-'.date("Ymd");
  /* Strip out all new lines and convert tabs to spaces. */
  $bookmark_file = str_replace(
    array("\r", "\n", "\t")
  , array('', '', ' ')
  , $bookmark_file
  );
  /* Convert any text between <DD> and a new tag to one line with <BR> tags. */
  $bookmark_file = preg_replace_callback(
    '@<dd>(.*?)(<A|<\/|<DL|<DT|<P)@mis'
  , function($match) {
  ...
  $bookmark_file = preg_replace('/>(\s*?)</mis', ">\n<", $bookmark_file);
  /* Remove unnecessary html tags */
  $bookmark_file = preg_replace(
    '/(<!DOCTYPE|<META|<!--|<TITLE|<H1|<P)(.*?)\n/i'
  , ''
  , $bookmark_file
  );
  /* Remove any surrounding whitespace thats left. */
  $bookmark_file = trim($bookmark_file);
  /* Shift <DD> tags onto the same line as the preceding <A> */
  $bookmark_file = preg_replace('/\n<dd/', '<dd', $bookmark_file);
  // echo '<PRE>';print_r(htmlspecialchars($bookmark_file));die;
  ...
  foreach($lines as $line_no => $line) {
    ...
    if(preg_match('/<a/i', $line, $match2)) {
      ...
      if(preg_match('/href="(.*?)"/i', $line, $match3)) {
        ...
        // $items[$i]['meta'] = kbm_meta($match3[1]);
      } else {
        $items[$i]['uri'] = '';
        // $items[$i]['meta'] = '';
      }
